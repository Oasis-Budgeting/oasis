...
## 2025-05-18 - Cross-Site Scripting (XSS) in AI Chat
**Vulnerability:** A Cross-Site Scripting (XSS) vulnerability existed in `ui/src/pages/AiAdvisor.jsx`. The application used React's `dangerouslySetInnerHTML` to render both user and assistant messages to allow for simple regex-based formatting (like bolding and lists), without first sanitizing the `msg.content`. This allowed any malicious HTML or JavaScript strings present in the input to be executed directly in the user's browser.
**Learning:** Directly inserting dynamic text from users or external APIs (like LLMs) into the DOM using `dangerouslySetInnerHTML` bypasses React's built-in XSS protection and creates an immediate attack vector.
**Prevention:** If formatting is needed, always apply a strong HTML sanitization utility (like `escapeHtml` that escapes `<`, `>`, `&`, `'`, and `"`) *before* applying custom markdown regex. For content that doesn't need HTML formatting (like user input), avoid `dangerouslySetInnerHTML` altogether and just render it using standard React `{}` braces.
