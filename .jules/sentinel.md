...
## 2024-05-24 - [Fix SSRF vulnerability in AI proxy routes]
**Vulnerability:** The AI chat endpoint `/chat` and `/models` accepted user-provided URLs in `base_url` without strict IP validation. An attacker could potentially send requests to internal network IPs, `localhost`, and internal metadata services (SSRF) bypassing simple string-matching blocklists by using different IP formats or subdomains.
**Learning:** Initial blocklist attempts were incomplete or suffered from false positives when relying on simple string matching (like `hostname.startsWith('10.')` blocking `10.mycompany.com`).
**Prevention:** Use Node.js built-in `net.isIP()` function after parsing the URL, ensuring that validation specifically targets the resolved IP address string, avoiding false matches on domain names, while continuing to restrict private and internal address blocks correctly.
