...
## 2026-03-08 - bcrypt Denial of Service (DoS) Vulnerability
**Vulnerability:** A bcrypt Denial of Service (DoS) vulnerability existed in `api/src/routes/auth.js`. The `/register`, `/login`, and `/reset-password` endpoints accepted passwords of unbounded length. Because bcrypt's computation time scales with the length of the input string, an attacker could send excessively long strings (e.g., thousands of characters) to exhaust the server's CPU resources, rendering the application unresponsive.
**Learning:** Security hashing algorithms like bcrypt are designed to be slow to prevent brute-force attacks. However, this property can be turned against the server if the input length isn't restricted. While bcrypt truncates inputs (often at 72 bytes), older implementations or specific bindings might still hash the entire string or take excessively long to process it before truncation. It is crucial to enforce length limits on computationally expensive operations at the application layer.
**Prevention:** Always enforce a maximum length limit on passwords (e.g., 72 characters) before any database checks or hashing operations.
