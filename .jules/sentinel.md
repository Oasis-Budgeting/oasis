...
## 2024-05-24 - Bcrypt Denial of Service (DoS) vulnerability via excessively long passwords
**Vulnerability:** The `/register`, `/login`, and `/reset-password` endpoints in `api/src/routes/auth.js` lacked password length validation. Because bcrypt is a CPU-intensive algorithm designed to be slow, passing an extremely long password (e.g., hundreds of thousands of characters) can cause the server to spend excessive CPU cycles hashing or comparing the password, potentially leading to a Denial of Service (DoS).
**Learning:** Bcrypt inherently truncates inputs after 72 bytes, so characters beyond that have no effect on security. However, before truncation (or depending on the library's internal handling), the string length check/conversion itself can cause overhead, and allowing unbounded input to an expensive cryptographic function is generally a dangerous pattern.
**Prevention:** Always enforce a maximum length limit on passwords (e.g., 72 characters) at the API validation layer before passing them to bcrypt.
