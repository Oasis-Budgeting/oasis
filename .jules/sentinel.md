...
## 2024-05-24 - Cross-Site Scripting (XSS) in AI Advisor
**Vulnerability:** The AI Advisor component (`ui/src/pages/AiAdvisor.jsx`) was rendering both user and AI-generated messages directly into the DOM using `dangerouslySetInnerHTML` without proper sanitization. This allowed XSS attacks if a user inputted malicious HTML/JS or if the LLM returned a payload containing malicious code.
**Learning:** Even internal or local LLM integrations can be an XSS vector if their output is implicitly trusted and injected into the DOM. User input must also never be injected as HTML if it's meant to be plain text.
**Prevention:** Use a custom `escapeHtml` utility to sanitize HTML entities (`&`, `<`, `>`, `"`, `'`) before parsing formatting (like markdown) with `dangerouslySetInnerHTML`. For pure text elements like user messages, rely on React's default text rendering rather than `dangerouslySetInnerHTML`.
