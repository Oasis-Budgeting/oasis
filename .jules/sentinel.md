...
## 2026-03-28 - Cross-Site Scripting (XSS) in AI Chat
**Vulnerability:** The AI chat page (`ui/src/pages/AiAdvisor.jsx`) suffered from a severe Cross-Site Scripting (XSS) vulnerability. It rendered user and assistant messages using React's `dangerouslySetInnerHTML` directly, allowing malicious input (like `<script>alert(1)</script>`) to be executed within the context of the user's session.
**Learning:** React safely handles plain text variables directly injected into JSX elements (e.g., `{msg.content}`). However, using `dangerouslySetInnerHTML` to allow custom regex formatting bypasses React's escaping mechanism. Both user messages and unescaped assistant messages were vulnerable to this oversight.
**Prevention:** Avoid `dangerouslySetInnerHTML` whenever possible. If it must be used for basic markdown formatting (e.g., bolding, bullet points), ensure the raw input is first sanitized using a robust utility (e.g., a custom `escapeHtml` function encoding `&`, `<`, `>`, `"`, and `'`) before applying formatting regexes. Do not use `dangerouslySetInnerHTML` for purely text-based fields like user inputs.
