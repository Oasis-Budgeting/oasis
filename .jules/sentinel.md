...
## 2025-03-13 - [Sentinel] Fix user enumeration and timing attack in login
**Vulnerability:** User enumeration and timing attack vulnerabilities in the `/login` endpoint (disclosed by discrepancies in failure messages and early-returns avoiding `bcrypt.compare` when no user exists).
**Learning:** Returning varied error messages ("Invalid credentials" vs "Invalid email or password") based on exact failure step helps attackers guess usernames. Similarly, an early `if(!user)` return bypasses password hashing, thus taking noticeably less time than valid logins and leaking username existence.
**Prevention:** Always use consistent, non-revealing error messages ("Invalid credentials"). Mitigate timing attacks by computing a dummy password hash offline, storing it, and passing it to `bcrypt.compare` if the user is not found, ensuring uniform process response time.
