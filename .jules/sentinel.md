...
## 2025-03-20 - Unauthenticated Metrics Endpoint Exposure
**Vulnerability:** The `/metrics` endpoint in the API (api/src/routes/metrics.js) was exposing global financial statistics (e.g., total balance, monthly income across all users) without any authentication. This constitutes a critical Information Disclosure vulnerability.
**Learning:** Internal observability endpoints (like Prometheus metrics) are often overlooked during security reviews because they don't perform actions, but they can easily leak sensitive aggregate data.
**Prevention:** Always secure internal monitoring, admin, or metrics endpoints behind proper authentication (e.g., a strong secret via Bearer token) and consider IP whitelisting or separate internal network bindings where applicable. Remember to use `crypto.timingSafeEqual` when comparing secrets to prevent timing attacks.
